      ...
      help: (pattern, name) ->
        ...
        if /^"/.test pattern
          if !getDefinition(name)
            @finish "Press enter to create a new window named %w", name.replace(/"/g, "")
          else
            @finish "Press enter to focus window %w.", name.replace(/"/g, "")
        else
...
#              openTab win
    ...
    merge:
      ...
      help: (name) ->
        return @finish 'Enter a defined window name.' if !name?
        withWindow name, (win) =>
